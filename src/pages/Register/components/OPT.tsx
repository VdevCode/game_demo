...
function OPT({ setOPT, data }: { setOPT: any; data: any }) {
  ...
  const handelSignUp = async () => {
    const res = await axios.post(configs.api.register, data);
              ...
              <main className="relative py-2 px-[10%] min-h-[40vh] flex-1 w-full landscape:flex landscape:flex-col">
                <div className="flex-1">
                  <h1>
                    ...
                    <span className='mx-1 font-bold'>+{data.phone}</span>. Hãy nhập chúng ở đây nhé!
                  </h1>
                ...
                <div className="absolute bottom-0 right-0 left-0 px-[5%] landscape:flex landscape:justify-between landscape:translate-y-1/2">
                  <Button onClick={() => {}}>Gửi mã xác thực</Button>
                  {loading ? (
                    <Button>Đang tải</Button>
                  ) : (
                    <Button onClick={onOTPVerify}>Xác thực</Button>
                  )}
                </div>
              </main>
            </div>
